    ...
    for (;;)
    {
        ...
        }
        {
            ...
            inBuf + inPos, &inProcessedAfter, finishMode, &status);
                ...
                lzmaDecDecodeToBuf lzmaDecDecodeToBufPtr = (lzmaDecDecodeToBuf)(*(int *)(qosa_get_cp_addr() + CP_LZMA_DEC_API_INDEX*4));
                #else
...
PLAT_BL_UNCOMP_FLASH_TEXT int lzmaDecodeEc(unsigned char *outStream, unsigned int *outStreamLen, unsigned char *inStream, unsigned int inStreamLen, unsigned int dataType, unsigned int *lzmaUnpackLen)
{
    ...
    if(((unsigned int)decompBuffStartAddr<(unsigned int)outStream)&&((unsigned int)outStream <(unsigned int)(decompBuffStartAddr+AP_DECOMP_MEM_TOTAL_LEN)))
    {
        __asm volatile("movw r12, #0xeca6");
        __asm volatile("movt r12, #0xeca6");
...
*/
#ifdef UNIRTOS_PLATFORM_MODIFY
    pFlashXIPAddr = *(int *)((qosa_get_cp_size()-8)+qosa_get_cp_addr())+qosa_get_cp_addr();
#else
    ...
    /* if magicHdr is error, maybe the bin is not compressed */
    if(compBinHeader.magicHdr != COMPRESS_MAGIC_HEADER)
    {
        __asm volatile("movw r12, #0xecab");
        __asm volatile("movt r12, #0xecab");
        __asm volatile("str  r12, [r12]");
    }
...
PLAT_BL_UNCOMP_FLASH_TEXT __attribute__((noinline)) void decompressCpRamCodeHashCheckOs(int ramCodeType)
{
    unsigned int *asmbData = NULL;
    unsigned int *msmbData = NULL;
    unsigned int index = ramCodeType-1;
    ...
    UINT32 lzmaCurrCpTime1 = 0;
    unsigned int decompInfoFlag = 0;
    MwAalSha256Ctx_t  gLzmaSha256Ctx;
...
*/
    ...
    if(compBinHeader.magicHdr != COMPRESS_MAGIC_HEADER)
    {
        __asm volatile("movw r12, #0xecae");
        __asm volatile("movt r12, #0xecae");
        __asm volatile("str  r12, [r12]");
    }
...
//    if(pCompBinSectionInfoAddr[index].type != ramCodeType)
    ...
    if(apmuGetAPBootFlag()==AP_BOOT_FROM_POWER_ON)
    {
        ...
        {
            //sctInitLzma();           
            mwAalInitSha256(&gLzmaSha256Ctx, 0);
            ...
            if((APCP_HASH_MSMB_MEM_START_ADDR <= pCompBinSectionInfoAddr[index].origAddr)&&(pCompBinSectionInfoAddr[index].origAddr <= APCP_HASH_MSMB_MEM_END_ADDR ))
            {
                ...
                {
                    //RetValue = shaUpdateLzma(2, (uint32_t)pCompBinSectionInfoAddr[index].origAddr, (uint32_t)lzmaHashOut, pCompBinSectionInfoAddr[index].origLen, 1);
                    RetValue = mwAalUpdateSha256(&gLzmaSha256Ctx, (const uint8_t *)pCompBinSectionInfoAddr[index].origAddr, (uint8_t *)lzmaHashOut, (size_t)pCompBinSectionInfoAddr[index].origLen, 1);
                }
                else
                {
                    hashCount = pCompBinSectionInfoAddr[index].origLen/APCP_HASH_ONCE_MAX_LEN;
                    ...
                    {
                        //RetValue = shaUpdateLzma(2, (uint32_t)(pCompBinSectionInfoAddr[index].origAddr+i*APCP_HASH_ONCE_MAX_LEN), (uint32_t)lzmaHashOut, APCP_HASH_ONCE_MAX_LEN, 0);
                        RetValue = mwAalUpdateSha256(&gLzmaSha256Ctx, (const uint8_t *)(pCompBinSectionInfoAddr[index].origAddr+i*APCP_HASH_ONCE_MAX_LEN), (uint8_t *)lzmaHashOut, (size_t)APCP_HASH_ONCE_MAX_LEN, 0);
                    }
                    lengthLeft = pCompBinSectionInfoAddr[index].origLen%APCP_HASH_ONCE_MAX_LEN;

                    //RetValue = shaUpdateLzma(2, (uint32_t)(pCompBinSectionInfoAddr[index].origAddr+hashCount*APCP_HASH_ONCE_MAX_LEN), (uint32_t)lzmaHashOut, lengthLeft, 1);
                    RetValue = mwAalUpdateSha256(&gLzmaSha256Ctx, (const uint8_t *)(pCompBinSectionInfoAddr[index].origAddr+hashCount*APCP_HASH_ONCE_MAX_LEN), (uint8_t *)lzmaHashOut, (size_t)lengthLeft, 1);
                }
            }
            else
            {
                ...
                if(length <= CP_HASH_MEM_LEN)
                {
                    ...
                    //RetValue = shaUpdateLzma(2, (uint32_t)msmbData, (uint32_t)lzmaHashOut, pCompBinSectionInfoAddr[index].origLen, 1);
                    RetValue = mwAalUpdateSha256(&gLzmaSha256Ctx, (const uint8_t *)msmbData, (uint8_t *)lzmaHashOut, (size_t)pCompBinSectionInfoAddr[index].origLen, 1);
                }
                else
                {
                    hashCount = pCompBinSectionInfoAddr[index].origLen/CP_HASH_MEM_LEN;
                    for(i=0; i<hashCount; i++)
                    {
                        ...
                        //RetValue = shaUpdateLzma(2, (uint32_t)msmbData, (uint32_t)lzmaHashOut, CP_HASH_MEM_LEN, 0);
                        RetValue = mwAalUpdateSha256(&gLzmaSha256Ctx, (const uint8_t *)msmbData, (uint8_t *)lzmaHashOut, (size_t)CP_HASH_MEM_LEN, 0);
                    }
                    lengthLeft = pCompBinSectionInfoAddr[index].origLen%CP_HASH_MEM_LEN;
                    length = lengthLeft/sizeof(unsigned int);
                    asmbData = (unsigned int *)(pCompBinSectionInfoAddr[index].origAddr + CP_RAM_BASE_ADDR + hashCount*CP_HASH_MEM_LEN);
                    ...
                    //RetValue = shaUpdateLzma(2, (uint32_t)msmbData, (uint32_t)lzmaHashOut, lengthLeft, 1);
                    RetValue = mwAalUpdateSha256(&gLzmaSha256Ctx, (const uint8_t *)msmbData, (uint8_t *)lzmaHashOut, (size_t)lengthLeft, 1);
            ...
            for(i=0; i<APCP_HASH_OUT_MAX_LEN; i++)
            {
                ...
                {
                    apCompExecuteRet.cpDecompHashRet[index] = 0xCDE00001;
                    __asm volatile("movw r12, #0xecaf");
                    __asm volatile("movt r12, #0xecaf");
                    __asm volatile("str  r12, [r12]");
                    break;
                }
            }
            ...
            //sctDeInitLzma();
            mwAalDeinitSha256(&gLzmaSha256Ctx);
        }
    }
...
PLAT_BL_UNCOMP_FLASH_TEXT void decompressGetAddrInfo(void)
{
    ...
    #ifdef CORE_IS_AP
#ifdef UNIRTOS_PLATFORM_MODIFY
    pFlashXIPAddr = *(int *)((qosa_get_ap_size()-8)+qosa_get_ap_addr())+qosa_get_ap_addr();
#else
    ...
    if(compBinHeader.magicHdr != COMPRESS_MAGIC_HEADER)
    {
        __asm volatile("movw r12, #0xecac");
        __asm volatile("movt r12, #0xecac");
        __asm volatile("str  r12, [r12]");
    }
...
#ifdef RAMCODE_COMPRESS_EN
PLAT_BL_UNCOMP_FLASH_TEXT void decompressRamCodeCleanBuff(void)
{
    #ifndef FEATURE_BOOTLOADER_PROJECT_ENABLE
        extern void __fast_memset(UINT32 *dst, UINT32 value, UINT32 length);
        #ifdef CORE_IS_AP
        __fast_memset((UINT32 *)decompBuffStartAddr, 0, AP_DECOMP_MEM_RF_CALIB_MAX_LEN);
        #endif
        #ifdef CORE_IS_CP
        __fast_memset((UINT32 *)decompBuffStartAddr, 0, CP_DECOMP_MEM_RF_CALIB_MAX_LEN);
        #endif
    #endif
}
...
PLAT_BL_UNCOMP_FLASH_TEXT void decompressRamCodeFromBin(int ramCodeType)
{
    ...
    if((pCompBinSectionInfoAddr[index].ziped == 0x1)&&(pCompBinSectionInfoAddr[index].compressedLen > 0))
    {
        if(pCompBinSectionInfoAddr[index].compressedLen <= DECOMPRESS_DATA_LEN_ONCE)
        {
            ...
            compData = (unsigned char *)(pCompBinSectionInfoAddr[index].compressedAddr + BOOTLOADER_FLASH_LOAD_ADDR);
                ...
                #ifdef CORE_IS_AP
                #ifdef UNIRTOS_PLATFORM_MODIFY
                compData = (unsigned char *)(pCompBinSectionInfoAddr[index].compressedAddr + qosa_get_ap_addr());
                #else
                ...
                #ifdef CORE_IS_CP
                #ifdef UNIRTOS_PLATFORM_MODIFY
                compData = (unsigned char *)(pCompBinSectionInfoAddr[index].compressedAddr + qosa_get_cp_addr());
                #else
        ...
        else
        {
            ...
            for(k=0; k<lzmaDecompMaxCount; k++)
            {
                ...
                compData = (unsigned char *)(pCompBinSectionInfoAddr[index].compressedAddr + BOOTLOADER_FLASH_LOAD_ADDR + k*DECOMPRESS_DATA_LEN_ONCE);
                    ...
                    #ifdef CORE_IS_AP
                    #ifdef UNIRTOS_PLATFORM_MODIFY
                    compData = (unsigned char *)(pCompBinSectionInfoAddr[index].compressedAddr + qosa_get_ap_addr() + k*DECOMPRESS_DATA_LEN_ONCE);
                    #else
                    ...
                    #ifdef CORE_IS_CP
                    #ifdef UNIRTOS_PLATFORM_MODIFY
                    compData = (unsigned char *)(pCompBinSectionInfoAddr[index].compressedAddr + qosa_get_cp_addr() + k*DECOMPRESS_DATA_LEN_ONCE);
                    #else
    ...
    else
    {
        ...
        if(pCompBinSectionInfoAddr[index].origLen > 0)
        {
            ...
            flashData = (unsigned int *)(pCompBinSectionInfoAddr[index].compressedAddr + BOOTLOADER_FLASH_LOAD_ADDR);
                ...
                #ifdef CORE_IS_AP
                #ifdef UNIRTOS_PLATFORM_MODIFY
                flashData = (unsigned int *)(pCompBinSectionInfoAddr[index].compressedAddr + qosa_get_ap_addr());
                #else
                ...
                #ifdef CORE_IS_CP
                #ifdef UNIRTOS_PLATFORM_MODIFY
                flashData = (unsigned int *)(pCompBinSectionInfoAddr[index].compressedAddr + qosa_get_cp_addr());
                #else
    ...
    if(res != 0)
    {
        __asm volatile("movw r12, #0xeca9");
        __asm volatile("movt r12, #0xeca9");
        __asm volatile("str  r12, [r12]");
    }
    #endif
    #ifdef CORE_IS_CP
    ...
    if(res != 0)
    {
        __asm volatile("movw r12, #0xeca8");
        __asm volatile("movt r12, #0xeca8");
        __asm volatile("str  r12, [r12]");
    }
    #endif
