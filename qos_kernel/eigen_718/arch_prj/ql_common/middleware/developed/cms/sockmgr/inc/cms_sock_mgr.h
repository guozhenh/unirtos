...
#define  CMS_SOCK_TOTAL_MGR_DL_LENGTH_MAX  (26*1024)   //max 26K DL PDU buffer
#else
...
typedef enum {
    ...
    */
    ...
    CME_REF_CR_SOCK_INVALID_PARAM              = 518, //Invalid parameters
    CME_REF_CR_SOCK_UNKNOWN_ERROR              = 550, //Unknown error
    CME_REF_CR_SOCK_TCPIP_NOT_USED             = 551, //Tcpip not used
    CME_REF_CR_SOCK_TCPIP_BEEN_USED            = 552, //Tcpip have been used
    CME_REF_CR_SOCK_TCPIP_NOT_CONNECT          = 553, //Tcpip not connet
    CME_REF_CR_SOCK_SOCK_CREATE_FAIL           = 554, //Socket creation failed
    CME_REF_CR_SOCK_SOCK_BIND_FAIL             = 555, //Socket bind failed
    CME_REF_CR_SOCK_SOCK_LISTEN_FAIL           = 556, //Socket listen failed
    CME_REF_CR_SOCK_CONNECT_REJ                = 557, //Socket connect reject
    CME_REF_CR_SOCK_CONNECT_TIMEOUT            = 558, //Socket connect timeout
    CME_REF_CR_SOCK_CONNECT_FAIL               = 559, //Socket connect fail(other abnormal)
    CME_REF_CR_SOCK_SOCK_WRITE_FAIL            = 560, //Socket write failed
    CME_REF_CR_SOCK_SOCK_READ_FAIL             = 561, //Socket read failed
    CME_REF_CR_SOCK_ACCEPT_FAIL                = 562, //Socket accept failed
    CME_REF_CR_SOCK_CLOSE_TIMEOUT              = 563, //Socket clsoe timeout,server no response
    CME_REF_CR_SOCK_CLOSE_FAIL                 = 564, //Socket clsoe fail(other abnormal)
    CME_REF_CR_SOCK_PDP_NOT_ACTIVE             = 570, //PDP not active
    CME_REF_CR_SOCK_PDP_ACTIVE_FAIL            = 571, //PDP active failed
    CME_REF_CR_SOCK_PDP_DEACTIVE_FAIL          = 572, //PDP deactive failed
    CME_REF_CR_SOCK_APN_NOT_CONFIGURED         = 575, //APN not configured
    CME_REF_CR_SOCK_PORT_BUSY                  = 576, //Port busy
    CME_REF_CR_SOCK_NOT_SUPPORT_IPV4_V6        = 577, //Not support IPV4/IPV6
    CME_REF_CR_SOCK_DNS_PARSE_FAIL             = 580, //DNS parse failed
    ...
    CME_REF_CR_SOCK_MEM_NOT_ENOUGH             = 588, //Memory not enough

    /*here is a internal error for showing URC to applayer*/
    CME_REF_CR_SOCK_SOCK_ID_HAS_BEEN_USED      = 800, //Socket Identity has been used
    CME_REF_CR_SOCK_CONNECTID_NOT_BIND_CID     = 801, //ConnectId not bind with cid
    CME_REF_CR_SOCK_SOCK_OPTION_SET_FAIL       = 802, //TCPIP option set fail
    CME_REF_CR_SOCK_MGR_WRITE_IS_ONGOING       = 803, /*in this case the send length is litter than expect length, cms will
                                                   continue to send the buffer,and NOT directly response SEND CNF until
                                                   all raw data sent out*/
	CME_REF_CR_SOCK_SOCK_HAS_BEEN_CLOSE        = 804, //Socket has been closed

    CME_REF_CR_SOCK_ERROR_MAX = 1024,
}CmsRefCrSockErrorCode;
...
typedef struct CmsSockMgrContext_Tag{
    ...
    UINT8   dlCachePkgNum: 7 ;/*Cal the DL buff save in cache num¡ê?limited in CMS_SOCK_TOTAL_MGR_DL_PKG_MAX*/
    UINT16  dlCacheCalLen;  /*Cal the DL buffer has Malloc but Not send out buffer size*/


    ip_addr_t localAddr;
    UINT32    connectStartTicks;
    UINT32    connectTimeout; //For tcp connect
    UINT32    dlMaxLen;
...
typedef struct CmsSockMgrConnDlDropArg_Tag{
    UINT16 fromPort;
    UINT16 dropLength;
    ip_addr_t fromAddr;
}CmsSockMgrConnDlDropArg;
...
typedef struct CmsRefSockCfgParam_Tag
{
    ...
    UINT8                               bTcpKeepAliveWithRaiFlag:1 ;
    UINT8                               bresv:1;
    CmsRefSockTcpKeepAliveParam         tcpKeepAliveParam;
}CmsRefSockCfgParam;  // 40 bytes
...
INT32 cmsSockMgrConnectSocket(INT32 fd, UINT8 domain, UINT16 remotePort, ip_addr_t *remoteAddr, UINT32 timeout);
...
void cmsSockMgrCallDlDropEventCallback(CmsSockMgrContext* gMgrContext, ip_addr_t *remoteAddr, UINT16 remotePort, UINT16 dropLength);
...
INT32  cmsSockMgrProcUlPendingList(CmsSockMgrContext *pSockMgr);

INT32  cmsSockMgrProcAllUlPendingList(void);
