...
typedef enum MidWareATChanCfgEnum_Tag
{
    ...
    MID_WARE_AT_CHAN_S3_CFG,                //ATS3
    MID_WARE_AT_CHAN_S4_CFG,                //ATS4
    MID_WARE_AT_CHAN_S5_CFG,                //ATS5
    MID_WARE_AT_CHAN_S12_CFG,               //ATS12
    MID_WARE_AT_CHAN_DCD_CFG,               //AT&C
    MID_WARE_AT_CHAN_DTR_CFG,               //AT&D
    MID_WARE_AT_CHAN_RESET_CFG,             //AT&F
    MID_WARE_AT_CHAN_STORE_CFG,             //AT&W
    MID_WARE_AT_CHAN_DEFAULTS_CFG,          //ATZ
    MID_WARE_AT_CHAN_CALL_CFG,              //ATX
    MID_WARE_AT_CHAN_VERBOSE_CFG,           //ATV
    MID_WARE_AT_CHAN_CGREG_RPT_CFG,         //CGREG
    ...
    MID_WARE_AT_CHAN_CNEC_RPT_CFG,         /*+CNEC=[<n>], enabled unsolicited reporting of error code sent by the network for different management:
                                            ...
                                            */
    MID_WARE_AT_CHAN_CRING_RPT_CFG,         //AT+CRC for URC: +CRING
    MID_WARE_AT_CHAN_CLIP_RPT_CFG,          //AT+CLIP for URC: +CLIP
    ...
    MID_WARE_AT_CHAN_RING_RPT_CFG,          //RING
    MID_WARE_AT_CHAN_NO_CARRIER_RPT_CFG,    //NO CARRIER
...
typedef struct _SIG_EPAT_MW_CFG_AT_CHAN_1_CONFIG
{
    ...
    UINT32  cnecRptMode      : 4;       /*+CNEC=[<n>], enabled unsolicited reporting of error code sent by the network for different management:
                                        ...
                                        * Note: The other 3 bits on the head are not used on CAT1: CS_MM/GPRS_MM/GPRS_SM
                                        */
    UINT32  cringRptMode     : 1;       //+CRING
    UINT32  clipRptMode      : 1;       //+CLIP
    UINT32  colpRptMode      : 1;       //+COLP
    UINT32  cvhuMode         : 1;       //+CVHU
    UINT32  ciregUrcMode     : 2;       //+CIREGU
    UINT32  ccwaRptMode      : 1;       //+CCWA

    UINT32  ringRptMode      : 1;       //RING
...
#ifdef UNIRTOS_PLATFORM_MODIFY
/*
 * AT CMD channel config 2
 * paramId: MW_CFG_AT_CHAN_1_CONFIG_2/...
*/
typedef struct _SIG_EPAT_MW_CFG_AT_CHAN_1_CONFIG_2
{
    UINT32 chanId       : 4;   //PSDecoderChann, currently, only one channel used for ATCMD: PS_CHANNEL_MODEM

    UINT32 s3Value      : 7;   //ATS3 (Max Value is 127[7 bits])
    UINT32 s4Value      : 7;   //ATS4 (Max Value is 127[7 bits])
    UINT32 s5Value      : 7;   //ATS5 (Max Value is 127[7 bits])
    UINT32 s12Value     : 8;   //ATS12 (Max Value is 255[8 bits])
    UINT32 dcdControl   : 1;   //AT&C (Max Value is 1[1 bits])
    UINT32 dtrControl   : 2;   //AT&D (Max Value is 2[2 bits])
    UINT32 resultCall   : 3;   //ATX (Max Value is 4[3 bits])
    UINT32 verboseFlag  : 1;   //ATV (Max Value is 1[1 bits])
    UINT32 cgregRptMode : 2;   //CGREG (Max Value is 2[2 bits])

    UINT64 rsvd0        : 35;  //VALUE is 0
} MWCfgAtChanConfig_2;         // 10 bytes
