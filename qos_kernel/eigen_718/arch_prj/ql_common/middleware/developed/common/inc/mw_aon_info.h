...
#include "psdial.h"
#if defined (FEATURE_PS_REF_CR_EPS_AT_ENABLE) || defined (FEATURE_PS_REF_CR_TCPIP_SOCK_AT_ENABLE)
#include "atec_ref_cr_tcpip.h"
#endif
...
typedef enum MWAonMipcallState_Tag {
    MW_AON_MIPCALL_INACTIVE        = 0,
    MW_AON_MIPCALL_ACTIVATING      = 1,
    MW_AON_MIPCALL_ACTIVE          = 2,
    MW_AON_MIPCALL_DEACTIVATING    = 3,
}MWAonMipcallState;
...
#if defined(FEATURE_REF_AT_QR_ENABLE) || defined(FEATURE_REF_AT_CR_ENABLE)
...
#ifdef UNIRTOS_MODEM_MODIFY
/*
* cscs 
*/

typedef struct MWAonCfgCSCSParam_Tag
{
...
* cnmi state
*/
typedef struct MWAonCfgCNMIParam_Tag
{
    UINT8   mode;           /* <mode>:the control mode for buffering URC */
    UINT8   mt;             /* <mt>:SMS-DELIVER URC mode */
    UINT8   bm;             /* Protocol Identifier */
    UINT8   ds;             /* Data coding scheme */
    UINT8   bfr;            /* Data coding scheme */
    UINT8   rsvd0;
    UINT16  rsvd1;

}MWAonCfgCNMIParam;
#endif /* UNIRTOS_MODEM_MODIFY */
...
#ifndef FEATURE_PS_REF_CR_MIPCALL_ENABLE
    UINT32      rsvd : 30;
#else
    UINT32      mipcallStateBitMap : 30; /* MWAonMipcallState, cid value[1-15], every cid use 2 bits, e.g. cid1 use bit2 and bit3, cid 2 use bit4 and bit5 */
#endif
...
#ifdef UNIRTOS_PLATFORM_MODIFY
    /*
    * AT channel Aon config 2, ParamId: MW_CFG_AT_CHAN_1_CONFIG/...
    */
    MWAonAtChanConfig_2 atChanConfig_2[MID_WARE_USED_AT_CHAN_NUM];  //10*4 bytes

    // Maximum can be expanded by 60 bytes(Information from EIGEN's Jason)
#endif /* UNIRTOS_PLATFORM_MODIFY */
...
#ifdef UNIRTOS_MODEM_MODIFY
    /*
    * nw aon cnmi param cfg
    */
    MWAonCfgCNMIParam        mwCnmiParamCfgAonInfo;

    /*
    * nw aon cscs param cfg
    */
    MWAonCfgCSCSParam        mwCscsParamCfgAonInfo;
#endif /* UNIRTOS_MODEM_MODIFY */
...
#ifdef UNIRTOS_PLATFORM_MODIFY
/**
  \fn           void mwAonGetAllAtChanConfig(MWAonAtChanConfig *pAtChanConfig, MWAonAtChanConfig_2 *pAtChanConfig_2)
  \brief        get AON atChanConfig
  \param[out]   MWNvmCfgAtChanConfig  *pAtChanConfig
  				MWAonAtChanConfig_2 *pAtChanConfig_2
  \returns      void
*/
void mwAonGetAllAtChanConfig(MWAonAtChanConfig *pAtChanConfig, MWAonAtChanConfig_2 *pAtChanConfig_2);
...
#ifdef UNIRTOS_PLATFORM_MODIFY
/**
  \fn           void mwAonSetAndSaveAtChanConfig(MWAonAtChanConfig *pAtChanConfig, MWAonAtChanConfig_2 *pAtChanConfig_2)
  \brief        Set and save atChanConfig
  \param[in]    MWNvmCfgAtChanConfig *pAtChanConfig
  				MWAonAtChanConfig_2 *pAtChanConfig_2
  \Return       void
*/
void mwAonSetAndSaveAllAtChanConfig(MWAonAtChanConfig *pAtChanConfig, MWAonAtChanConfig_2 *pAtChanConfig_2);
#else /* UNIRTOS_PLATFORM_MODIFY */
/**
...
void mwAonGetCnmiParamCfig(MWAonCfgCNMIParam *pMwAonCnmiParamCfg);

void mwAonSetCnmiParamCfgAndSave(MWAonCfgCNMIParam *pMwAonCnmiParamCfg);

void mwAonGetCscsParamCfig(MWAonCfgCSCSParam *pMwAonCscsParamCfg);

void mwAonSetCscsParamCfgAndSave(MWAonCfgCSCSParam *pMwAonCscsParamCfg);
...
void mwAonSetMipcallStateByCid(UINT8 cid, MWAonMipcallState state);
MWAonMipcallState mwAonGetMipcallStateByCid(UINT8 cid);
#endif
