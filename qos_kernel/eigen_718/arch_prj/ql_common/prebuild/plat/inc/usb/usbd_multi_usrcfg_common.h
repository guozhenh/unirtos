...
//this file may be built under different option 
//libfeat_usb_build_opt.h for feat selectable lib ojbect build
//linkfeat_usb_build_opt.h for link-only object build 
...
#ifdef SIMU_QUEC_CFG
//for sim quec the vcom desc was set to ccinst_setting_vcom_subtp1_2ep, ccinst_setting_vcom_subtp2_3ep
//redefine VCOM_CCINST_CASE_SEL
#undef VCOM_CCINST_CASE_SEL
#define VCOM_CCINST_CASE_SEL VCOM_CCINST_CUST_CASE
#endif
...
#ifdef UNIRTOS_PLATFORM_MODIFY
#define VCOM_CCINST_SUBTP0_INHERENT_CNT 1
#else /* UNIRTOS_PLATFORM_MODIFY */
#define VCOM_CCINST_SUBTP0_INHERENT_CNT 0
#endif /* UNIRTOS_PLATFORM_MODIFY */

#define VCOM_CCINST_SUBTP1_2EP_CNT 1
#if defined (UNIRTOS_OPEN_MODE_SUPPORT) || defined(__QUECTEL_PROJECT_EG800ZCN_LG__)
#define VCOM_CCINST_SUBTP2_3EP_CNT 3
#else  /* defined (UNIRTOS_PLATFORM_MODIFY) || defined(__QUECTEL_PROJECT_EG800ZCN_LG__) */
#define VCOM_CCINST_SUBTP2_3EP_CNT 2
#endif /* defined (UNIRTOS_PLATFORM_MODIFY) || defined(__QUECTEL_PROJECT_EG800ZCN_LG__) */
#endif
...
//real used dev cnt MULTI_CUST_DEV_CNT_MAX-1, the last one for empty mark
#define MULTI_CUST_DEV_CNT_MAX 7
...
typedef struct {
    uint8_t elem_cnt;
    //real used dev cnt MULTI_CUST_DEV_CNT_MAX-1, the last one for empty mark    
    multidev_custom_element elem_arr[MULTI_CUST_DEV_CNT_MAX];
...
typedef struct {
    ...
    uint8_t load_cnt;   
    uint8_t filter_cnt;
    multidev_load_element elem_arr[MAX_SUPPORT_FUNCTION_RESOURCE];
}multidev_loadres_info_st;

extern uint32_t multidev_get_max_load_cnt(void);

extern uint8_t usbcustom_multidev_cfg_delcare(void);

extern uint8_t usbcustom_multidev_cfg_reset(void);
extern multidev_custom_info_st*  usbcustom_multidev_cfg_update(void);
extern uint8_t usbcustom_multidev_cfg_end(multidev_loadres_info_st *p_multidev_load);
...
typedef struct 
{
  uint8_t (*multidev_delcare)(void);   
    
  uint8_t (*multidev_reset)(void);   
  multidev_custom_info_st* (*multidev_update)(void); 
  uint8_t (*multidev_end)(multidev_loadres_info_st *p_multidev_load);
