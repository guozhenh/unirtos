...
typedef enum
{
    SCT_USB_DEINT   = 0,    /* when USB plug out */
    SCT_USB_INIT    = 1,    /* USB is OK (EP enumed), and SCT could sent data to USBC TxFIFO, when USB plug in */
    SCT_USB_RESET   = 2,    /* USB is reseted, SCT USB channel should be reset, and wait USB init(OK) again. */
    SCT_USB_SUSPEND = 3,    /* USB is suspended */
    SCT_USB_RESUME  = 4,    /* USB is resume */
}SctUsbStateEvent;
...
typedef enum
{
    ...
    UTFW_USB_SUSPEND= SCT_USB_SUSPEND,
    UTFW_USB_RESUME = SCT_USB_RESUME
}SctUsbStateEvent;
 ...
 * Comment:
******************************************************************************/
void UtfwUsbStateEventInIsr(UtfwUsbStateEvent usbState, UINT32 extras);
...
/******************************************************************************
 * UtfwGetEpResumeFifoNotEmptyCount
 * Description: get consective count/times of case: the EP FIFO not empty when EP resume
 * input:   UINT8 epId
 * output:  UINT16
 * Comment:
******************************************************************************/
UINT16 UtfwGetEpResumeFifoNotEmptyCount(UINT8 epId);
