...
typedef enum
{
    ...
    ccinst_setting_hid_subtp0_inhrnt =1,
...
typedef struct tag_ccinst_hid_sub_desc
{
    uint8_t protocol;
    uint16_t input_report_size;
    uint16_t output_report_size;  
    uint16_t clsdt_hid_subdesc_size;
    uint16_t report_desc_size;
    uint8_t *p_clsdt_hid_subdesc;    
    uint8_t *p_report_desc;    
}ccinst_hid_sub_desc_st;

typedef struct tag_ccinst_hid_setting
{
    ccinst_setting_base_st bs_set;
    ccinst_hid_sub_desc_st sub_desc_data;
    uint8_t map_flag : 1;
    uint8_t desc_assigned : 1;
    uint8_t desc_parsed : 1;
    uint8_t binded          :1;
    uint8_t itriep_valid : 1;
    uint8_t itroep_valid : 1;
    uint8_t rsv_1: 2;

    uint8_t itriep_num: 4;
    uint8_t itroep_num: 4;
    uint8_t intf_base_idx: 4;
    uint8_t  rsv_2:4;    
    ...
    uint8_t intf_str_id;
    uint16_t desc_src_size;
    uint8_t cust_deflt_rptbuf_exist;//if exist use default input report data and size default 
    uint8_t *p_cust_deflt_rptbuf;
    
    usb_endpoint_descriptor_st  * p_itriep_d;
    usb_endpoint_descriptor_st  * p_itroep_d;
    uint8_t *p_desc_src;
    
}ccinst_hid_setting_st;
