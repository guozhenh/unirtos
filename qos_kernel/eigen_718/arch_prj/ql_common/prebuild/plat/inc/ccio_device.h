 ...
 *                    MACROS                                                  *
 *----------------------------------------------------------------------------*/
#define CCIO_DEV_TX_LIST_APPEND(dev, hd, tl, cnt)   \
        if (((CcioDevice_t*)(dev))->txTail == NULL)\
        {\
            EC_ASSERT(((CcioDevice_t*)(dev))->txHead == NULL, ((CcioDevice_t*)(dev))->txHead, cnt, 0x1234);\
            ((CcioDevice_t*)(dev))->txHead  = (hd);\
            ((CcioDevice_t*)(dev))->txTail  = (tl);\
            ((CcioDevice_t*)(dev))->txCount = (cnt);\
        }\
        else\
        {\
            EC_ASSERT(((CcioDevice_t*)(dev))->txHead != NULL && ((DlPduBlock_t *)(((CcioDevice_t*)(dev))->txTail))->pNext == NULL,\
                      ((CcioDevice_t*)(dev))->txHead, ((DlPduBlock_t *)(((CcioDevice_t*)(dev))->txTail))->pNext, 0x5678);\
            ((DlPduBlock_t *)(((CcioDevice_t*)(dev))->txTail))->pNext = (hd);\
            ((CcioDevice_t*)(dev))->txTail   = (tl);\
            ((CcioDevice_t*)(dev))->txCount += (cnt);\
        }
...
typedef enum _EPAT_CcioDevExecFlagsTag
{
    ...
    CCIO_DEF_ADD_DEV_CMPLT,
    CCIO_DEF_DEL_DEV_CMPLT,
    CCIO_DEF_ALT_DEV_CMPLT,
...
typedef struct
{
    uint8_t   llsn;
    uint8_t   rsvd[3];
    void     *syncId;    /* 'osSemaphoreId_t' or 'osEventFlagsId_t' */
}CcioDeaAddDevCmplt_t;
...
typedef struct
{
    ...
    uint8_t   rsvd[3];
    void     *syncId;    /* 'osSemaphoreId_t' or 'osEventFlagsId_t' */
    void     *dlpdu;     /* DlPduBlock_t, pending list */
    uint32_t  extras;
}CcioDeaDelDevCmplt_t;
...
typedef enum _EPAT_CcioDevStatusFlagsTag
{
    ...
    CCIO_DSF_UART_BEGIN = 0x0D5F1000,
    CCIO_DSF_AUTO_BAUDRATE,
    CCIO_DSF_WAIT_TX_DONE,
    CCIO_DSF_SET_TX_DONE,
    CCIO_DSF_IPR_FLUSH,
    CCIO_DSF_UART_END = 0x0D5F1FFF,

    CCIO_DSF_SPI_BEGIN = 0x0D5F2000,
    CCIO_DSF_RBUF_ALERT,
    CCIO_DSF_SPI_END = 0x0D5F2FFF,

    CCIO_DSF_I2S_BEGIN = 0x0D5F3000,
    CCIO_DSF_I2S_END = 0x0D5F3FFF,

    CCIO_DSF_RADIO_BEGIN = 0x0D5F5000,
    CCIO_DSF_ULPDU_FREE,
    CCIO_DSF_DLPDU_FREE,
    CCIO_DSF_WAN_MEDIA_NOTIF,
    CCIO_DSF_RADIO_END = 0x0D5F5FFF,
...
typedef struct
{
    ...
    uint8_t   xtag;      /* some private info */
    uint8_t   xtras;     /* some other private info */
    ...
    {
        void  *tail;
        void  *extras;   /* some essential info that depends  */
    }u;
#endif
    struct CcioDevice *chdev;
}CcioDevOutXfer_t;
...
typedef struct
{
    uint16_t  flushCnt;  /* flush the outdated data if necessary */
    uint16_t  flushSti;  /* flush the outdated data if necessary */
    uint32_t  baudRate;
    struct CcioDevice *chdev;
...
typedef struct
{
    ...
    uint8_t  altCos;    /* refer to (CCIO_CHAN_MSG_CF_COS0/1/2) >> 16 */
    uint8_t  rsvd[2];
    struct CcioDevice *chdev;
}CcioDsaDevAlt_t;
...
/* CCIO_VOLTE_HANDLE_CODEC */
typedef struct
{
    uint32_t volumeLevelHandle : 3; // 0~7
    uint32_t speakerVolGain    : 5; // 0~31
    uint32_t speakerVolVal     : 7; // 0~100
    uint32_t micVolGain        : 5; // 0~31
    uint32_t micVolVal         : 8; // 0~256
    uint32_t rsvd              : 4;
}codecParam_t;
 ...
 */
CcioDevice_t* ccioFindDevice(uint32_t devId);
/**
