...
typedef enum _EPAT_CMI_DEV_PRIM_ID_TAG
{
    ...
    CMI_DEV_IMS_CFUN_IND,                   //CmiDevImsCfunInd

    CMI_DEV_DETECT_EVENT_IND,               //CmiDevDetectEventInd

    CMI_DEV_SET_CLOUD_IMEI_REQ,             //CmiDevSetCloudImeiReq
    CMI_DEV_SET_CLOUD_IMEI_CNF,             //CmiDevSetCloudImeiCnf

    CMI_DEV_GET_BLOCKPLMN_LIST_REQ,         //CmiDevGetBlockPlmnListReq
    CMI_DEV_GET_BLOCKPLMN_LIST_CNF,         //CmiDevGetBlockPlmnListCnf

    CMI_DEV_SET_JDC_PARA_REQ,               //CmiDevSetJdcParaReq
    CMI_DEV_SET_JDC_PARA_CNF,
    CMI_DEV_GET_JDC_PARA_REQ,
    CMI_DEV_GET_JDC_PARA_CNF,               //CmiDevGetJdcParaCnf
    CMI_DEV_JAMM_DETECT_IND,                //CmiDevJammDetectInd

    CMI_DEV_STOP_WIFISCAN_REQ,              //CmiDevStopWifiScanReq
    CMI_DEV_STOP_WIFISCAN_CNF,              //CmiDevStopWifiScanCnf
    CMI_DEV_WIFISCAN_IND,                   //CmiDevWifiScanInd
...
******************************************************************************/
#define CMI_DEV_SUPPORT_MAX_FREQ_NUM    32
...
}CmiDevSetCiotFreqReq; // total 136 bytes
...
typedef UINT8   CmiRrcBarEvent_t;
typedef enum CmiRrcBarEventTag
{
    CMI_RRC_BAR_EVENT_NONE  = 0,
    CMI_RRC_BAR_EVENT_T300_EXPIRED = 1,
    CMI_RRC_BAR_EVENT_RADIO_LINK_FAILURE = 2,
    CMI_RRC_BAR_EVENT_IDLE_UNRELIABLE = 3,
    CMI_RRC_BAR_EVENT_RESEL_NCELL_CRC_ERROR = 4,
}CmiRrcBarEvent;
...
typedef struct CmiDevSetExtCfgReq_Tag
{
    ...
    BOOL    smallUlTbsOptPresent;
    BOOL    smallUlTbsOpt;              //whether enable small ul tbs opt.
    UINT16  ccmRrsvd2;
    ...
    */
    ...
    BOOL    t3402Opt;
    BOOL    antiDetectPresent;
    BOOL    antiDetect;

    BOOL    clrForbiddenTacPst;
    BOOL    clrForbiddenTac;
    BOOL    attemptHplmnPst;
    BOOL    attemptHplmn;

    BOOL    forbTacTimerPst;
    UINT16  forbTacTimerS;      /* User defined forbidden TAC timer value in seconds */
    UINT8   emmRsvd2;

    BOOL    setOperatorIdPst;
    UINT8   operatorId;
    BOOL    quickSearchPresent;
    UINT8   quickSearch;

    BOOL    sameIpAddrProcPresent;
    UINT8   sameIpAddrProc;
    BOOL    bEitPcoConexistPresent;
    BOOL    bEitPcoConexist;

    BOOL    cellPreferPst;
    UINT8   cellPreferMode;         //0 - disable cellCampPerfer; 1 - enable cellCampPerfer for FDD; 2 - enable cellCampPerfer for TDD
    UINT8   cellPreferScoreAdd;     //range [0-100], PHY will add the score to FDD/TDD frequency point score during frequency and band fast cell search
    UINT8   emmRsvd4;
    ...
    UINT16  relaxMonitorEvalTime; // range (3~300), unit: second. relaxed monitoring evaluate time (TSearchDeltaP)
    UINT8   errcRsvd0[2];
    ...
    BOOL    l2ExtSnCapaBitmapPresent;
    UINT8   l2ExtSnCapaBitmap;
    BOOL    autoApnPresent;
    BOOL    autoApn;
	
    BOOL    bRrcBarTimePresent;
    CmiRrcBarEvent_t   rrcBarEvent;
    UINT16  rrcBarTime;

    UINT8   errcRsvd3;
    BOOL    bUltraHighSpeedPresent;
    BOOL    ultraHighSpeed;
    BOOL    bEnableSibRevOptPresent;
    BOOL    bEnableSibRevOpt;
    BOOL    barAfterRrcTryNumPresent;
    UINT8   barAfterRrcTryNum;
    BOOL    asUeCapaBitmapPresent;
    UINT32  asUeCapaBitmap;
...
/*
* used for AT+ECCFG = 'RrcBarTime', rrcBarEvent, rrcBarTime
*/
typedef struct CmiDevRrcBarTimeCfg_Tag
{
    //Used for cell barTime when RRC T300 expire
    UINT16              barTimeT300Expire;
    //Used for cell barTime when occur RLF in Connection state
    UINT16              barTimeRLF;
    //Used for cell barTime where reception of MIB/SIB1 fail(CRC, decode) during cell reselection
    UINT16              barTimeReselCrcErr;
    //Used for cell barTime when receive 'unrealiable' indication, and meanwhile UE has not yet collected SIB3 or SIB5 in QualityFirst mode
    UINT16              barTimeIdleUnreliable;
}CmiDevRrcBarTimeCfg;
...
typedef struct CmiDevGetExtCfgCnf_Tag
{
    ...
    UINT8   operatorId;
    UINT8   cellPreferMode;
    UINT8   cellPreferScoreAdd;
    BOOL    quickSearch;

    /* ERRC */
    UINT8   ueCfgDataInactTimer;        /* ERRC data inactivity timer, in seconds */
    UINT8   relaxMonitorDeltaP;         /* relaxed monitor parameter, in DB, 0 - 15 */
    UINT16  relaxMonitorEvalTime;       // range (3~300), unit: second. relaxed monitoring evaluate time (TSearchDeltaP)
    UINT8   relVersion;                 /* release version: 9, 13 - 14 */
    ...
    UINT32  asUeCapaBitmap;
    CmiDevRrcBarTimeCfg     rrcBarTime;
...
/******************************************************************************
 * CMI_DEV_GET_BLOCKPLMN_LIST_REQ
 *  AT+ECBLOCKPLMNLIST?
******************************************************************************/
#define CMI_DEV_BLOCK_PLMN_LIST_NUM    4
typedef CamCmiEmptySig CmiDevGetBlockPlmnListReq;

typedef enum CmiBlockPlmnCause_TAG
{
    CMI_DEV_BLOCK_PLMN_NONE = -1,                    /* default value */
    CMI_DEV_BLOCK_PLMN_OTHERS = 0,                   /* other cause */
    CMI_DEV_BLOCK_PLMN_VOICE_NOT_AVAILABLE = 1,      /* voice centirc,but voice service not available */
    CMI_DEV_BLOCK_PLMN_MAX_REGISTRATION_FAILURE = 2, /* 1.reject cause #42
                                                        2.voice centric,combined attach fail max times
                                                        3.TS-24.30 5.5.2.3.4 UE abnormal case b) in a MT detach and start T3402 */
    CMI_DEV_BLOCK_PLMN_PRIORITY = 3,                 /* PDN reject 3 times */
    CMI_DEV_BLOCK_PLMN_SIM_PROHIBIT = 4              /* FPLMN in SIM card */
}CmiBlockPlmnCause;

typedef struct CmiDevBlockPlmnListInfo_Tag
{
    INT8            blockCause;     //CmiBlockPlmnCause
    UINT16          remainTimerS;   //remain timer value in seconds
    CmiNumericPlmn  plmn;
}CmiDevBlockPlmnListInfo;

typedef struct CmiDevGetBlockPlmnListCnf_Tag
{
    UINT8                   numOfList;
    UINT8                   rsvd[3];
    CmiDevBlockPlmnListInfo blockPlmnList[CMI_DEV_BLOCK_PLMN_LIST_NUM];
}CmiDevGetBlockPlmnListCnf;
...
typedef enum CmiGetExtStatusEnum_TAG
{
    ...
    CMI_DEV_GET_MUESTATS_RADIO,  //AT+MUESTATS="radio"
    CMI_DEV_GET_MUESTATS_CELL,   //AT+MUESTATS="cell"
    CMI_DEV_GET_MUESTATS_SBAND,   //AT+MUESTATS="sband"
    CMI_DEV_GET_MUESTATS         //AT+MUESTATS="all"
}CmiGetExtStatusEnum;
...
typedef enum CmiDevWifiScanRptMode_Enum
{
    CMI_DEV_WIFISCAN_SYN_RPT  = 0,        //wifi info report in CMI CNF
    CMI_DEV_WIFISCAN_ASYN_RPT             //wifi info report in CMI IND (URC)
}CmiDevWifiScanInfoRptMode;
...
typedef struct CmiDevGetBasicCellListInfoReq_Tag
{
    ...
    UINT32      freqList[CMI_DEV_SUPPORT_MAX_FREQ_NUM];
    UINT8       bandList[CMI_DEV_SUPPORT_MAX_BAND_NUM];
...
typedef struct CmiDevGetBasicCellListInfoCnf_Tag
{
    ...
    CmiDevSCellBasicInfo    sCellInfo;  //36 bytes
    CmiDevNCellBasicInfo    nCellList[CMI_DEV_NCELL_INFO_CELL_NUM]; //32*20 bytes
...
typedef struct CmiDevSetWifiSacnReq_Tag
{
    ...
    UINT8   rptMode;            //CmiDevWifiScanInfoRptMode
    UINT8   rsvd[3];
...
/******************************************************************************
 * CMI_DEV_STOP_WIFISCAN_REQ
******************************************************************************/
typedef CamCmiEmptySig CmiDevStopWifiScanReq;
...
/******************************************************************************
 * CMI_DEV_STOP_WIFISCAN_CNF
******************************************************************************/
typedef CamCmiEmptySig CmiDevStopWifiScanCnf;

/******************************************************************************
 * CMI_DEV_WIFISCAN_IND
******************************************************************************/
typedef CmiDevSetWifiScanCnf CmiDevWifiScanInd;
...
typedef enum CmiDevImsDeregCause_enum
{
    ...
    CMI_IMS_DEREG_SET_BAND          = 6     /* ECBAND */
}CmiDevImsDeregCause;
...
typedef enum CmiDevRrcEventTypeEnum_Tag
{
    ...
    CMI_DEV_PHY_SCANNING_BAND = 4,          /* indicate the band which is currently being scanned during cell search */
    CMI_DEV_DRX_CYCLE_UPDATED = 5,          /* report current drx cycle and configured from NW or AT */
    CMI_DEV_RRC_IDLE_MEAS_THRESHOLD = 6,    /* report RRC idle intra and inter measure threshold */
    CMI_DEV_RRC_IDLE_MEAS_ACTION = 7,       /* report RRC idle measurement status */
...
typedef struct CmiDevErrcDrxCycleUpdatedTag
{
    //the drxCycleMs is configured in SIB2(defaultPagingCycle), range {320ms, 640ms, 1280ms, 2560ms}
    UINT16                          drxCycleMs;
    //the drxCycle is modified by AT+ECCFG="UserDrxCycle", range {320ms, 640ms, 1280ms, 2560ms, 5120ms}
    //If the userDrxCycleMs is 0, indicate the AT "UserDrxCycle" is not configured
    UINT16                          userDrxCycleMs;
}CmiDevErrcDrxCycleUpdated;

typedef struct CmiDevErrcIdleMeasThresholdTag
{
    /* NOTE:
     * 1> the Idle Measuremnt threshold is only used for 3GPP 36.304-5.2.4.2 "Measurement rules for cell re-selection"
     * 2> if the sIntraSearchP and sNonIntraSearchP is 0, it means 'Cell Lock is enabled' or 'DisableNCellMeas' is enabled
     */

    //if serving cell rsrp <= sIntraSearchP, UE will perform INTRA-frequency measurement
    INT16                           sIntraSearchP;
    //if serving cell rsrp <= sNonIntraSearchP, UE will perform INTER-frequency measurement
    INT16                           sNonIntraSearchP;

    //indicate if NW configure s-IntraSearchQ
    BOOL                            sIntraSearchQPresent;
    //indicate if NW configure s-NonIntraSearchQ
    BOOL                            sNonIntraSearchQPresent;
    //if sIntraSearchQ is present and serving cell rsrq <= sIntraSearchQ, UE will perform INTRA-frequency measurement
    INT16                           sIntraSearchQ;
    //if sNonIntraSearchQ is present and if serving cell rsrq <= sNonIntraSearchQ, UE will perform INTER-frequency measurement
    INT16                           sNonIntraSearchQ;
    UINT16                          rsvd;
}CmiDevErrcIdleMeasThreshold;

typedef enum CmiDevNcellMeasTypeTag
{
    CMI_DEV_NCELL_MEAS_NONE     = 0,
    //previous UE is only perfrom serving cell measurement, now start idle INTRA or INTER frequency measurement
    CMI_DEV_NCELL_MEAS_START    = 1,
    //previous UE is perfrom INTRA or INTER frequency measurement, now stop all IDLE INTRA and INTER frequency measurement
    CMI_DEV_NCELL_MEAS_STOP     = 2,
}CmiDevNcellMeasType;

typedef struct CmiDevErrcIdleMeasActionTag
{
    UINT8                           measAction; //CmiDevNcellMeasType
    UINT8                           rsvd[3];
}CmiDevErrcIdleMeasAction;
...
typedef struct CmiDevErrcEventInd_Tag
{
    ...
    union {
        ...
        //report when received first SIB2 or drxCycle is changed for AT+ECCFG="UserDrxCycle"
        CmiDevErrcDrxCycleUpdated           drxInfo;
        //report RRC idle intra and inter measurement threshold
        CmiDevErrcIdleMeasThreshold         idleMeasThresh;
        //report RRC idle neighbour measurement status (start/stop neighbour cell measurement)
        CmiDevErrcIdleMeasAction            idleMeasAction;
   };
}CmiDevErrcEventInd;
...
typedef struct CmiDevImsCfunInd_Tag
{
    UINT8   cfun;       //CmiFuncValueEnum
    UINT8   rsvd0;
    UINT16  rsvd1;
}CmiDevImsCfunInd;
...
/******************************************************************************
 * CMI_DEV_SET_CLOUD_IMEI_REQ
******************************************************************************/
typedef struct CmiDevSetCloudImeiReq_Tag
{
    UINT8   imei[15];
    UINT8   rsvd;
}CmiDevSetCloudImeiReq;

/******************************************************************************
 * CMI_DEV_SET_CLOUD_IMEI_CNF
******************************************************************************/
typedef CamCmiEmptySig CmiDevSetCloudImeiCnf;

/******************************************************************************
 * CMI_DEV_SET_JDC_PARA_REQ
******************************************************************************/
typedef struct CmiDevSetJdcParaReq_Tag
{
    UINT8           mode; //0: Disable Jamming detection; 1:Enable Jamming Detection.
    UINT8           rsvd[3];
    UINT8           Tdetect;
 ...
 * CMI_DEV_SET_JDC_PARA_CNF
******************************************************************************/
typedef CamCmiEmptySig CmiDevSetJdcParaCnf;

/******************************************************************************
 * CMI_DEV_GET_JDC_PARA_REQ
******************************************************************************/
typedef CamCmiEmptySig CmiDevGetJdcParaReq;

/******************************************************************************
 * CMI_DEV_GET_JDC_PARA_CNF
******************************************************************************/
typedef struct CmiDevGetJdcParaCnf_Tag
{
    UINT8           mode; //0: Disable Jamming detection; 1:Enable Jamming Detection.
    UINT8           rsvd[3];
    UINT8           Tdetect;
...
typedef struct CmiDevJammDetectInd_Tag
{
    UINT8           jammDetectRslt;
    UINT8           rsvd[3];
}CmiDevJammDetectInd;
