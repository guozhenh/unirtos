...
typedef struct NetMgrLanChannelTcpipDataPathStatus_Tag
{
    ...
    UINT8           lanActiveIp4Cid;
    UINT8           lanActiveIp6Cid;
    UINT16          rsvd;
 ...
 * NM_ADDR_IPV6_ID: 8~15:0 0~7:id
*/
typedef struct NmIpv6Prefix_Tag
{
    ip6_addr_t    ipv6Prefix;           /* ipv6 prefix info */
    ip6_addr_t    ipv6GateWay;           /* ipv6 gateway info */
    INT32         routerLeftValidTime;    //the left valid life time of the ipv6 default router(seconds),
    UINT32        prefixLeftValidTime;    //the left valid life time of the ipv6 prefix(seconds)
}NmIpv6Prefix;  //40 bytes
...
typedef struct NmIfConfiguration_Tag
{
    ...
    NmIpv6Prefix *pIp6Prefix; //only set if the addrType of ipv6Addr is NM_ADDR_IPV6_ID, and it will free in NetMgrLinkUp() API
...
typedef struct NmNetIpv6Info_Tag{
    ...
    ip6_addr_t      ipv6GateWay;   /* the default gateway of ipv6*/
...
typedef enum NmLanNetifStatusChangeCause_Tag
{
    ...
    NM_LAN_STATUS_CHANGE_WAN_IP6_RA_SUCC,
 ...
 * psDlIpPkgProcFunc(UINT8 cid, DlPduBlock *pPduHdr)
*/
