...
/**
  \fn        trimGetFtDevVer( void )
  \brief     used  to set current 718 type to gvar
             in order to seperate VOLTE support version
             byte 57 bit[7:5] as L3
             byte 58 bit[1:0] as H2
             MSB(byte58 bit 1) is VOLTE support flag
  \note
 */
void  trimSetFtDevVerGvar( void );


/**
  \fn        trimGetFtDevVer( void )
  \brief     used  to get current 718 type from gvar
             in order to seperate VOLTE support version
             byte 57 bit[7:5] as L3
             byte 58 bit[1:0] as H2
             MSB(byte58 bit 1) is VOLTE support flag
  \note
 */
uint8_t trimGetFtDevVer( void );


/**
  \fn        uint8_t trimGetPGMVer(void)
  \brief     get ATE FT_PGM_VERSION
  \note
 */
...
/**
  \fn        void trimGetSecureBit(uint8_t *readVal)
  \brief     read secure bit, called in BRInfoInit
  \param[in] N/A
  \param[out]
    uint8_t temp[4];
    trimGetSecureBit(temp); //read secure en bit
    memcpy(&PlatBits, &temp[2], 1);
    GBRInfo.JTAGDisable = PlatBits.JtagDis;
    GBRInfo.SecureBootEnabled  = PlatBits.SecurityBootEn;
    GBRInfo.DbgDisable  = PlatBits.DebugDis;
    GBRInfo.DownloadDisable = PlatBits.DownloadDis;
  \note
 */
void trimGetSecureBit(uint8_t *readVal);
