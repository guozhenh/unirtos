...
#ifdef FEATURE_REF_CR_ENABLE
#define PLAT_CFG_RAW_FLASH_RSVD_SIZE    7
#else
...
__PACKED_STRUCT _plat_config_raw_flash
{
    ...
    */
    ...
    #ifdef FEATURE_REF_CR_ENABLE
    uint8_t mledCfg;        // configure for AT+MLED

    uint8_t mlpmCfg;        // configure for AT+MLPMCFG="sleepind"

    uint8_t delaySleep;     // configure for AT+MLPMCFG="delaysleep"
    #endif
...
typedef enum _plat_config_id
{
    ...
    #ifdef FEATURE_REF_CR_ENABLE
    PLAT_CONFIG_ITEM_MLED_CFG,               /**< configure for AT+MLED */
    PLAT_CONFIG_ITEM_MLPM_CFG,               /**< configure for AT+MLPMCFG="sleepind" */
    PLAT_CONFIG_ITEM_DELAYSLEEP_CFG,         /**< configure for AT+MLPMCFG="delaysleep" */
    #endif
} plat_config_id_t;
...
uint32_t BSP_GetFsBuadratevalue(void);

uint32_t unirtos_get_cpuuid(void);
