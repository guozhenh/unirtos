...
/**
  \fn	 uint32_t PAD_pinMuxRead(uint32_t paddr)
  \brief Read whether pin is now in mux mode
  \return pin corresponding mux mode
 */
uint32_t PAD_pinMuxRead(uint32_t paddr);

/**
  \fn	 uint32_t PAD_pinPullRead(uint32_t paddr)
  \brief Read whether pin is now in up-pull or down-pull
  \return pin corresponding pull mode
 */
uint32_t PAD_pinPullRead(uint32_t paddr);
