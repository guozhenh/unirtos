...
#ifndef _GPIO_EC7XX_H
#define _GPIO_EC7XX_H
...
/**
  \fn    uint32_t GPIO_dirRead(uint32_t port, uint16_t pin)
  \brief Read whether GPIO is now in input or output mode
  \param[in] port    GPIO number (0, 1, ...)
  \param[in] pin     GPIO pin number
  \return            GPIO corresponding pin working mode
 */
uint32_t GPIO_dirRead(uint32_t port, uint16_t pin);
