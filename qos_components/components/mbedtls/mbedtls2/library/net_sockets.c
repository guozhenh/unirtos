...
//
#elif defined(QUECTEL_OPEN_MODE_SUPPORT)
...
#else /* QUECTEL_OPEN_MODE_SUPPORT */
                         ...
                         const char *port, int proto, uint8_t cid )
{
    ...
    if(*host == '['){//handle ipv6 address
        hostLen = strlen(host);
        newHost = (char*)qosa_malloc(hostLen-1);
        memset(newHost, 0, hostLen-1);
        memcpy(newHost, host+1, hostLen-2);
        if( getaddrinfo( newHost, port, &hints, &addr_list ) != 0 ){
    ...
    if(newHost != NULL) mbedtls_free(newHost);
...
int mbedtls_net_send(void *ctx, const unsigned char *buf, size_t len)
{
    ...
    do
    {
        ...
        preSelTime=qosa_get_system_tick_cnt();
        ret = select(fd+1, NULL, &writeFs, NULL, &tv);
        if(ret>0)
        {
            hasSend = send(fd, (buf + len - waitToSend), waitToSend, MSG_DONTWAIT);
            QLOGD( "%d bytes data has sent to server", hasSend);
            if(hasSend > 0)
            {
                waitToSend -= hasSend;
            }
            ...
            {
                QLOGD("send failed (errno:%d)", errno);
                return MBEDTLS_ERR_NET_SEND_FAILED;
            }

            QLOGD("preSelTime: %d,current tick %d", preSelTime,qosa_get_system_tick_cnt());
            passedTime=(qosa_get_system_tick_cnt()-preSelTime)>0?(qosa_get_system_tick_cnt() - preSelTime):(0xFFFFFFFF - preSelTime + qosa_get_system_tick_cnt());
