...
{
    mbedtls_ms_time_t current_ms;
    current_ms = (mbedtls_ms_time_t)qosa_get_system_time_milliseconds();
...
#elif defined(MBEDTLS_PSA_DRIVER_GET_ENTROPY)
#include <stdlib.h>
#include <string.h>
#include <stdio.h>
                                 ...
                                 unsigned char *output, size_t output_size)
{
    ...
    if(output_size <= intlen)
    {
        rvalue = rand();
        memcpy(output, (void *)&rvalue, output_size);
        *estimate_bits = 8 * output_size;
    }
    else
    {
        for(i=0 ;i < (output_size/intlen); i++)
        {
            rvalue = rand();
            memcpy(output+(i*intlen), (void *)&rvalue, intlen);
        }

        if((output_size%intlen) > 0)
        {
            rvalue = rand();
            memcpy(output+(i*intlen), (void *)&rvalue, output_size%intlen);
        }
        *estimate_bits = 8 * output_size;
    }
    return 0;
