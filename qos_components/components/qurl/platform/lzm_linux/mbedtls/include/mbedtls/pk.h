 ...
 */
#if defined(MBEDTLS_USE_PSA_CRYPTO) && defined(PSA_WANT_KEY_TYPE_ECC_PUBLIC_KEY) && !defined(MBEDTLS_ECP_C)
 ...
 */
    int
    mbedtls_pk_verify(mbedtls_pk_context *ctx, mbedtls_md_type_t md_alg, const unsigned char *hash, size_t hash_len, const unsigned char *sig, size_t sig_len);
 ...
 */
    int mbedtls_pk_wrap_as_opaque(mbedtls_pk_context *pk, mbedtls_svc_key_id_t *key, psa_algorithm_t alg, psa_key_usage_t usage, psa_algorithm_t alg2);
#endif /* MBEDTLS_USE_PSA_CRYPTO */
